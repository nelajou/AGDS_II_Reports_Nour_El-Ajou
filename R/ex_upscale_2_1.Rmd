...
print(combined_plot)
```
Looking at the density and the boxplots based on geographical distance, we can see that the target variable doesn't change strongly dependent on the respective clusters.
...
print(combined_plot)
```
We can see already visually looking at the densities and tha boxplots, that the Leave Nitrogen values don't differ widely between the different environmental clusters. This is similar to the boxplots of the clusters based on geographical distance. 
...
```
considering that we have only 5 Datapoints for each CV, we can see that R-Squared and RMSE are very similar distributed for the geographical and the environmental clustering. Hence, both models seem to be similar in their extrapolating power, in the interest of finding a model which can be used on a global scale.
...
Looking at the RMSE and the R-Squared , the Model done through random shows the smalllest standard error and the higher R-Squared, followed by the Environmental CV and the Spatial CV, which shows the highest error and the smallest R-Squared. 
...
One possible explenation would be that the predictors chosen, annal mean temperature and precipitation don't have a high enough feature importance. This could definitely be for two reasons: 
...
- feature importance 
-aber sogar wema aui predictors nimmt chas di abfucke wege dischtanz.

- boxplots bespräche.

- RSquared boxplots bespräche
