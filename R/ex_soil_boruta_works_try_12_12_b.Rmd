...
binary (0,1) encoded. We are interested to check if we have roughly as many ones as zeros in our dataset, which would be desirable. 

The reason for this is that, e.g. we would have mostly zeros, an simple algorithm (like one ignoring all covariates ) would always return a zero for new predictions.
...
```{r}
summary(df_full$waterlog.100 == 1)
summary(df_train$waterlog.100 == 1)

```
```{r}

share_of_postives_full <-  as.numeric(summary(df_full$waterlog.100 == 1)[3])/  ( (as.numeric(summary(df_full$waterlog.100 == 1)[2]) ) + 
   (as.numeric(summary(df_full$waterlog.100 == 1)[3]) ) )

cat("the percentage of positives is equal to ", share_of_postives_full, "in the full dataset")
```
...
cat("the percentage of positives is equal to ", share_of_postives, "in the training dataset")
```

the same goed for the training dataset.
...
conf_table <- as.data.frame( round(cbind(treshholds, true_neg, false_neg,true_pos, false_pos,   false_pos_rate, true_pos_rate), 3)
)
...
- Tuning part, is accuracy really  the right metric, or which one was used in 
the first forrest?
- Anser treshold question. 


# questions for class
vszone?
what could be problem for vmnet?
look at the metric in the hyperparameter tuning, discuss it with beni stocker.
