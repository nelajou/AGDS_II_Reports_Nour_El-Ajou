...
Looking at the RMSE and the R-Squared , the Model with random CV shows the smallest standard error and the higher R-Squared, followed by the environmental CV and the Spatial CV, which shows the highest error and the smallest R-Squared. 
...
The mild difference between the RMSE of the environmental and the spatial CV on the other hand, is somewhat surprising. Since we expect predictors in our model to have a significant importance, the idea is that generating the CV folds based on predictor space, should correct the clustering of the sampling data even more than the folds based on geographical distance. Consequently, we would expect the RMSE of the Environmental CV to be higher than the one of the geographical CV. However the opposite is the case in the data and this difference also persists when changing the random seed. 

One possible explanation for the difference would be that the predictors chosen, annal mean temperature and precipitation don't have a high enough feature importance. This could definitely be for two reasons: 
