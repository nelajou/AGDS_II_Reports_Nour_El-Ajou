...
dim(df_full)
```
We see that we have 1052 observations of 103 variables.

```{r}
head(df_full)
```
While the first 14 columns denote soil features, together with the labeling coordinates and the timestamp, the others denote environmental features.

### Spat Raster

We use SpatRaster files in order for our Predictors, here is an example, measuring slope for the defined Area. 
...
```
We train the random forest using the ranger package
...
print(rf_basic)
```
Our forest has 500 trees, mtry = 9 and a OOB of 21.16

### Prediction - Basic Random Forest

The following Code is used to generate predictions.
```{r}
...
```{r}
cat("the precision for the basic model is ",round(prec_basic,3))
```

### f1 = 2TP / (2TP + FP + FN) - Basic Forest
```{r}
cat("the f1 for the basic model is ",round(f1_basic,3))
...
```{r}
cat("the percentage of positives is equal to ", round(share_of_postives_train,3), "in the training dataset")
```
...
```
We see that roughely a third of all predictors were rejected, and a bit more thant that were confirmed. 


## 5.2.2 Train Boruta Random Forest
...
In a next step, we can train the same Random forest model, but only with the predictors that have been "confirmed".

```{r}
print(rf_bor)
```
Again, we have 400 Trees, but a mtry of 605 and a OOB prediction Error of 20.17
...
```{r}
cat("the precision for the boruta model is ", round(prec_bor,3))
```

### f1 = 2TP / (2TP + FP + FN) - Boruta
```{r}

 cat("the f1 for the boruta model is ", round(f1_bor,3))

```

## 5.2.4 Model Comparison - Basic Forest vs. Boruta Forest
...
For tuning, the Caret library is used, allowing to check different ranges for the hyperparameters used combined. The following code was used:
...
### comparison table for hyperparameters

we can illustrate the different hyperparameters in a table. 
```{r}
hyper.param.table
```
Comment: The optimal values vary quite strong when changing the random seed, so this shouldn't matter to much. This is also in lin e with the findings of Schratz et. al (2019), who pointed out that hyperparameter tuning has negligible effects for Random Forest models. 


## 5.3.2 Train Tuned Boruta Random Forest
...
Again, we can print our random forest. This time the Hyperparameters are set to the optimal ones. 
```{r}
...
Again, we can show the different metrics on a table for comparison. This Time for all models. 

```{r}
diag_table_3
```
Considering only the prediction error, the tuned Boruta model performs better than the untuned boruta model. Considering all other indicators, the tuned model performs better. 
...
In order to plot the AOC curve, a new dataframe is set together, which containes every point of the curve constructed.
```{r}
head(conf_table)
```
...
```
The red vertical lines show two possible treshholds, which could be chosen. 
...
```{r}
cat ("The treshhold returning the highest F1 would be ", max_treshold_F1, "with a a corresponding F1 of ",round(max(conf_table$f1),3) )
```
...
Type I Errors, while Type II Errors are not very crucial. I.e. we only want to measure the effect of the drug given it really does exist (minimize false positives) and we accept rejecting the use of the drug in some cases, even though it could help (we have more false negatives in return)
